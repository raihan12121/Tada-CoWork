...
export const ProviderSettings: React.FC = () => {
  ...
  return <div className="max-w-3xl mx-auto p-8 w-full">
    ...
    <div className="bg-[#161b22] border border-[#30363d] rounded-xl p-5 space-y-4">
      ...
      <label className="block text-xs text-gray-300">Provider<select value={provider} onChange={e => setProvider(e.target.value as Provider)} className="mt-1 w-full bg-[#0d1117] border border-[#30363d] rounded-lg p-2 text-sm text-white"><option value="openai">OpenAI API</option><option value="openai_codex">ChatGPT subscription via Codex CLI</option><option value="anthropic">Anthropic API</option><option value="anthropic_claude">Claude subscription via Claude Code CLI</option><option value="gemini">Google Gemini API</option><option value="ollama">Ollama (local)</option><option value="lm_studio">LM Studio (local)</option><option value="offline_heuristic">Offline preview (no AI)</option></select></label>
      {local ? <label className="block text-xs text-gray-300"><Server className="inline w-3 h-3 mr-1" />OpenAI-compatible endpoint<input value={endpoint} onChange={e => setEndpoint(e.target.value)} placeholder={provider === 'ollama' ? 'http://127.0.0.1:11434/v1' : 'http://127.0.0.1:1234/v1'} className="mt-1 w-full bg-[#0d1117] border border-[#30363d] rounded-lg p-2 text-sm text-white" /></label> : provider !== 'offline_heuristic' && provider !== 'openai_codex' && provider !== 'anthropic_claude' && <label className="block text-xs text-gray-300">API key<input type="password" value={apiKey} onChange={e => setApiKey(e.target.value)} placeholder="Enter provider API key" className="mt-1 w-full bg-[#0d1117] border border-[#30363d] rounded-lg p-2 text-sm text-white" /></label>}
